...
import { generateUIScreenshot } from './services/visualizer';
...
export default function App() {
  ...
  const handleVisualize = async () => {
    ...
    const img = await generateUIScreenshot();
    if (img) setUiImage(img);
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="text-[10px] font-bold uppercase tracking-widest px-4 py-1.5 border border-orange-500/30 rounded-full hover:bg-orange-500/10 transition-colors disabled:opacity-50"
            >
              {generatingImage ? "Generating..." : "Visualize UI"}
            </button>
        ...
        <AnimatePresence>
          {uiImage && (
            <motion.div 
              ...
              onClick={() => setUiImage(null)}
            >
              <div className="relative max-w-5xl w-full aspect-video rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-orange-500/20">
                <img src={uiImage} alt="DeepGuard UI Visualization" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                <button className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/50 backdrop-blur-md flex items-center justify-center text-white hover:bg-white/10 transition-colors">
                  ✕
                </button>
              </div>
            </motion.div>
